...
TEST reads_a_producer(void) {
    ...
    ASSERT_EQ(FR_OK, fr_project_read("test/fixtures/producer/tiestone.json", &project, &err));
...
TEST finds_a_module_and_its_requires(void) {
    fr_project project; fr_error err;
    fr_project_read("test/fixtures/producer/tiestone.json", &project, &err);
...
TEST keeps_a_source_block_of_any_kind(void) {
    ...
    fr_error err;
    ASSERT_EQ(FR_OK, fr_manifest_read("test/fixtures/consumer/tiestone-unknown-source.json", &manifest, &err));
...
TEST rejects_a_consumer_language_that_names_a_reserved_module_key(void) {
    fr_manifest manifest; fr_error err;
    ASSERT_EQ(FR_ERR, fr_manifest_read("test/fixtures/consumer-reserved-language/tiestone.json",
                                       &manifest, &err));
...
TEST rejects_an_unknown_schema(void) {
    fr_project project; fr_error err;
    ASSERT_EQ(FR_ERR, fr_project_read("test/fixtures/bad-schema/tiestone.json", &project, &err));
