...
static const char *ARTIFACT =
    "https://github.com/forebay/basekit/releases/download/5.0.0/terko.json";
static const char *FORK_ARTIFACT =
    "https://github.com/someone-else/basekit/releases/download/5.0.0/terko.json";
...
static const char *test_cache_root(void) {
    ...
    snprintf(root, sizeof root, "%s/terko_test_cache_%d", fr_test_temp_base(), fr_test_process_id());
    return root;
}
...
TEST rejects_an_oversized_cache_root_rather_than_truncating_it(void) {
    ...
    fr_test_set_env("TERKO_CACHE_DIR", oversized);
...
TEST round_trips_a_written_manifest(void) {
    ...
    ASSERT_EQ(0, fr_test_count_files(root, "terko.json.tmp"));
    ASSERT_EQ(1, fr_test_count_files(root, "terko.json"));
...
TEST keeps_two_artifacts_of_one_project_and_version_apart(void) {
    ...
    ASSERT_EQ(2, fr_test_count_files(root, "terko.json"));
...
TEST resolves_the_localappdata_fallback_when_no_override_is_set(void) {
    ...
    snprintf(expected_prefix, sizeof expected_prefix, "%s/terko/cache/", test_cache_root());
...
TEST resolves_the_xdg_cache_home_fallback_when_no_override_is_set(void) {
    ...
    ASSERT(path != NULL);
    char expected_prefix[600];
    snprintf(expected_prefix, sizeof expected_prefix, "%s/terko/", test_cache_root());
    ASSERT(strstr(path, expected_prefix) == path);
    free(path);
