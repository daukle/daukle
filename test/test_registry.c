...
TEST returns_a_plugin_registered_under_its_capability(void) {
    ...
    int called = 0;
    fr_source_plugin plugin = { "tiestone.source/test", fake_load, &called };
    fr_error err;
    ASSERT_EQ(FR_OK, fr_registry_add_source(registry, &plugin, &err));

    const fr_source_plugin *found = fr_registry_source(registry, "tiestone.source/test");
    ASSERT(found != NULL);
    fr_project project;
...
TEST returns_null_for_an_unregistered_capability(void) {
    fr_registry *registry = fr_registry_create();
    ASSERT(fr_registry_source(registry, "tiestone.source/absent") == NULL);
    fr_registry_destroy(registry);
    PASS();
}
...
TEST rejects_a_duplicate_capability(void) {
    ...
    ASSERT(strstr(err.message, "tiestone.source/test") != NULL);
    fr_registry_destroy(registry);
    PASS();
}
...
TEST keeps_source_and_language_spaces_separate(void) {
    fr_registry *registry = fr_registry_create();
    int called = 0;
    fr_source_plugin plugin = { "tiestone/x", fake_load, &called };
    fr_error err;
    fr_registry_add_source(registry, &plugin, &err);
    ASSERT(fr_registry_language(registry, "tiestone/x") == NULL);
    fr_registry_destroy(registry);
    PASS();
}
