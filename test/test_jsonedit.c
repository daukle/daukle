...
TEST creates_a_missing_nested_path(void) {
    ...
    ASSERT_EQ(FR_OK, fr_json_edit_set_string_array(PACKAGE, "terko.managed", "dependencies",
                                                   managed, 1, &out, &err));
    ASSERT_STR_EQ(
        "{\n"
        "  \"name\": \"stub-translator\",\n"
...
TEST removing_from_an_absent_object_changes_nothing(void) {
    char *out = NULL; fr_error err;
    ASSERT_EQ(FR_OK, fr_json_edit_remove(PACKAGE, "terko.managed", "dependencies", &out, &err));
    ASSERT_STR_EQ(PACKAGE, out);
    free(out);
    PASS();
}
