...
/* A missing or malformed file yields zero entries, which is what makes
   ...
   special case later. */
static size_t ledger_read(const char *derived_dir, ledger_entry *out, size_t capacity) {
    char path[1024];
    snprintf(path, sizeof path, "%s/%s", derived_dir, FR_DERIVED_LEDGER);

    char *text = NULL;
    fr_error err;
    if (fr_file_read_text(path, &text, &err) != FR_OK) return 0;
...
static int ledger_write(const char *derived_dir, ledger_entry *entries, size_t count) {
...
/* Written only when absent and never overwritten: a user who edited it had a
   reason. derived_dir is <derived root>/<toolchain>, so its parent is the
   derived root the .gitignore belongs beside. */
static void write_gitignore(const char *derived_dir) {
    const char *last_slash = strrchr(derived_dir, '/');
    const char *last_backslash = strrchr(derived_dir, '\\');
    const char *last = last_slash;
    if (last_backslash != NULL && (last == NULL || last_backslash > last)) last = last_backslash;
    if (last == NULL) return;

    size_t parent_length = (size_t) (last - derived_dir);
    char path[1024];
    if (parent_length + strlen("/.gitignore") + 1 > sizeof path) return;
    memcpy(path, derived_dir, parent_length);
    snprintf(path + parent_length, sizeof path - parent_length, "/.gitignore");
...
int fr_derived_apply(const char *derived_dir, const fr_generated_file *files, size_t count,
    ...
    size_t previous_count = ledger_read(derived_dir, previous, FR_DERIVED_MAX_FILES);
    ...
    for (size_t index = 0; index < count && result == FR_OK; index++) {
        char full[1024];
        snprintf(full, sizeof full, "%s/%s", derived_dir, files[index].path);
        ...
        if (!unchanged) {
            if (write && fr_file_write_text(full, files[index].text, err) != FR_OK) {
                result = FR_ERR;
                break;
            }
    ...
    for (size_t index = 0; index < previous_count && result == FR_OK; index++) {
        ...
        char full[1024];
        snprintf(full, sizeof full, "%s/%s", derived_dir, previous[index].path);
    ...
    if (result == FR_OK && write && next_count > 0) {
        write_gitignore(derived_dir);
        if (ledger_write(derived_dir, next, next_count) != FR_OK) {
            fr_error_set(err, "could not record what was generated in \"%s\"", derived_dir);
            result = FR_ERR;
        }
    }
