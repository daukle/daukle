...
static int cache_root_dir(char *out, size_t out_size, fr_error *err) {
    const char *override = getenv("TIESTONE_CACHE_DIR");
    ...
    if (local_appdata != NULL && local_appdata[0] != '\0') {
        return check_root_fit(snprintf(out, out_size, "%s/tiestone/cache", local_appdata), out_size, err);
    }
#else
    const char *xdg_cache_home = getenv("XDG_CACHE_HOME");
    if (xdg_cache_home != NULL && xdg_cache_home[0] != '\0') {
        return check_root_fit(snprintf(out, out_size, "%s/tiestone", xdg_cache_home), out_size, err);
    }
    const char *home = getenv("HOME");
    if (home != NULL && home[0] != '\0') {
        return check_root_fit(snprintf(out, out_size, "%s/.cache/tiestone", home), out_size, err);
    }
#endif

    fr_error_set(err, "no cache directory available: set TIESTONE_CACHE_DIR");
    return FR_ERR;
}
...
int fr_cache_path(const char *project, const char *version, const char *artifact,
    ...
    *out_path = NULL;
    ...
    size_t length = strlen(root) + 1 + strlen(project) + 1 + strlen(version) + 1
                  + strlen(artifact_id) + 1 + strlen("tiestone.json") + 1;
    char *path = malloc(length);
    if (path == NULL) {
        fr_error_set(err, "out of memory building cache path");
        return FR_ERR;
    }
    snprintf(path, length, "%s/%s/%s/%s/tiestone.json", root, project, version, artifact_id);
